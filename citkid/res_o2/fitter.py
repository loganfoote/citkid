...
from ..res.util import bounds_check, calculate_residuals
...
from citkid.res_o2 import guess
...
def fit_nonlinear_iq(f, z, bounds = None, p0 = None, fr_guess = None,
    ...
    fit_tau (bool): if False, tau is enforced from p0[8] to speed up fitting.
        If True, tau is fit.
    tau_guess (float or None): If float, overides p0[8].
    downward (bool): If True, fits the equation for a downward sweep. If
        False, fits for an upward sweep.
    ...
    if bounds is None:
        ...
        #                 fr,  Qr, amp,               phi, a, b,   i0,   q0,     tau
        bounds = ([np.min(f), 1e3, .01,        -np.pi / 2, 0, 0, -1e2, -1e2, -1.0e-6],
                  [np.max(f), 1e7,   1 - 1e-6,  np.pi / 2, 1, 1,  1e2,  1e2,  1.0e-6])
        for index in [1, 6, 7]:
...
def fit_util(p0, bounds, fit_tau, f, z_stacked, z, downward = True):
    ...
    """
    #             fr,   Qr, amp, phi, a, b, i0, q0, tau
    scaler = [100e-6, 1e-4,   1,   1, 1, 1,  1,  1, 1e6]
    ...
    if not fit_tau:
        # Fit with tau enforced from p0[8]
        tau = p0[8]
        bounds = np.array([bounds[0][:8], bounds[1][:8]])
        p0 = p0[:8]
        ...
        popt, pcov = optimize.curve_fit(fit_func, f, z_stacked, p0,
                                        bounds = bounds)
        popt = np.insert(popt, 8, tau)
        perr = np.sqrt(np.diag(pcov))
        perr = np.insert(perr, 8, 0)
    else:
        # Fit without enforcing tau
        def fit_func(x_lamb, a, b, c, d, e, f, g, h, i):
            return nonlinear_iq_for_fitter(x_lamb, a, b, c, d, e, f, g, h, i,
                                           downward)
        popt, pcov = optimize.curve_fit(fit_func, f, z_stacked, p0,
                                        bounds = bounds)
